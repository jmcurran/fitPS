...
test_that("predict.psFit uses fitted zero-inflation probability for ZIZ S predictions", {
  psData = makePSData(n = 1:3, count = c(8, 3, 1), type = "S")
  fit = list(
    psData = psData,
    pi = 0.2,
    shape = 1.25,
    fitted = c(S1 = 0.713, S2 = 0.084, S3 = 0.037),
    model = "ziz",
    method = "mle"
  )
  class(fit) = "psFit"

  prediction = predict(fit, newdata = 1)
  expected = (1 - fit$pi) * VGAM::dzeta(1, shape = fit$shape) + fit$pi
