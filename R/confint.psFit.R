...
#'   These values use the fitPS standard zeta shape parameter alpha, with shape > 1. VGAM uses a shifted parameter internally, equal to shape - 1. If a
...
confint.psFit = function(object, parm, level = 0.95, ...){
  ...
  if(object$model == "ziz"){
    ...
    profLik = Vectorize(
      function(bd){
        2 * (sum(dzetaStandard(rep(obsData, object$psData$data$rn), shape = bd, log = TRUE)) +
           object$fit$value) + cstar
      })

    zstar = qnorm((1 - level) * 0.5, lower.tail = FALSE)
    wald = object$shape + c(-1,1) * zstar * sqrt(object$var.shape)
    xmin = max(1 + sqrt(.Machine$double.eps), wald[1] - 0.5 * sqrt(object$var.shape))
