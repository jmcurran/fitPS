...
& 2 (1 - \Pr(Z < |Z_0|)).
\end{align*}
To do this in R we type:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlstd{alpha.1} \hlkwb{=} \hlstd{fit.lau}\hlopt{$}\hlstd{shape}
\hlstd{> }\hlstd{alpha.2} \hlkwb{=} \hlstd{fit.jackson}\hlopt{$}\hlstd{shape}
\hlstd{> }\hlstd{v.1} \hlkwb{=} \hlstd{fit.lau}\hlopt{$}\hlstd{var.shape}
\hlstd{> }\hlstd{v.2} \hlkwb{=} \hlstd{fit.jackson}\hlopt{$}\hlstd{var.shape}
\hlstd{> }\hlstd{se.diff} \hlkwb{=} \hlkwd{sqrt}\hlstd{(v.1} \hlopt{+} \hlstd{v.2)}
\hlstd{> }\hlstd{Z0} \hlkwb{=} \hlstd{(alpha.1} \hlopt{-} \hlstd{alpha.2)}\hlopt{/}\hlstd{se.diff}
\hlstd{> }\hlstd{P} \hlkwb{=} \hlnum{2} \hlopt{*} \hlstd{(}\hlnum{1} \hlopt{-} \hlkwd{pnorm}\hlstd{(}\hlkwd{abs}\hlstd{(Z0)))}
\hlstd{> }\hlstd{P}
\end{alltt}
\begin{verbatim}
[1] 0.23313
...
\end{kframe}
\end{knitrout}
We can see that the \emph{P}-value is 0.23 (2 d.p.). This is significantly larger than either 0.05, or 0.01. Based on this we would conclude that there is insufficient evidence to reject the null hypothesis of a common value of $\alpha$, and it may be sensible to combine data from these two surveys. We could have also used the theory of likelihood ratio tests \citep{wiki:lrt} to test this hypothesis, but that is beyond the scope of this article.
