...
The important architectural point is that the distribution-specific mathematics stays outside `fitPS`. The package supplies generic maximum-likelihood optimisation, low-dimensional numerical posterior integration, MCMC orchestration, and fitted-object machinery. The external model supplies its parameter names, observation mapping, probabilities, likelihood, priors, parameter constraints or transformations, and any model-specific starting information it needs.
...
The transform methods are used by generic MCMC. The one-dimensional numerical engine integrates on the natural parameter scale and therefore does not require a sampler-specific implementation.
...
S3method(modelToWorking,externalPoissonModel)
S3method(modelFromWorking,externalPoissonModel)
S3method(modelWorkingLogJacobian,externalPoissonModel)
```
...
This is a substantially stronger extensibility test than the ordinary Poisson model. The inner integral defining each likelihood contribution remains distribution-specific downstream mathematics, while `fitPS` owns the generic MLE optimizer and the outer Bayesian posterior integration or MCMC machinery.
...
### Add the Poisson-normal Bayesian contract

The two-parameter example uses a Normal prior for `mu` and a half-Normal prior for positive `sigma`. The Bayesian control reuses the finite MLE bounds; these give the two-dimensional numerical engine a finite integration rectangle. MCMC uses a log transform for `sigma`.

```{r poisson-normal-bayes-contract}
...
S3method(modelToWorking,externalPoissonNormalModel)
S3method(modelFromWorking,externalPoissonNormalModel)
S3method(modelWorkingLogJacobian,externalPoissonNormalModel)
```
...
The Poisson-normal example demonstrates something stronger. Its marginal probability and likelihood require an inner numerical integral, yet that distribution-specific calculation and all prior/parameter mathematics remain external. `fitPS` provides MLE optimisation, adaptive two-dimensional posterior cubature, generic MCMC, posterior summaries, and the standard `psFit` interface. No Poisson-normal branch is needed in `fit()`, `predict()`, `logLik()`, AIC, BIC, deviance, or the posterior engines.

That is the intended extension boundary: **new statistical models supply model mathematics; fitPS supplies common fitting, posterior orchestration, and fitted-object infrastructure.**
