---
title: "fitPS basics: fitting probability surveys"
output: rmarkdown::pdf_document
vignette: >
  %\VignetteIndexEntry{fitPS basics}
...
It is very simple to fit a zeta distribution to this data set. The current public interface uses `fit()` with an explicit zeta model descriptor.

```{r fit-roux}
fit = fit(roux, model = zetaModel())
```

The older `fitDist()` function remains as a deprecated compatibility wrapper, but new code should use `fit()`.
...
The older `fitZIDist()` function remains as a deprecated compatibility wrapper.
...
For a two-parameter zero-inflated zeta fit, `plotUncertainty()` provides the common parameter-uncertainty display used throughout fitPS. For an MLE fit it draws the established profile-likelihood confidence region. The numerical contour coordinates remain available from `confint()` when they are needed for downstream calculations rather than plotting.
...
The same plotting call is used after fitting by ordinary nonparametric bootstrap. The bootstrap fit stores its successful parameter replicates, so drawing the smoothed KDE confidence region does not rerun the bootstrap. This can matter because the resampling and repeated model fitting are substantially more expensive than drawing the contours.
...
The older `bootCI()` interface remains available for compatibility but is deprecated. New code should obtain the bootstrap distribution through `fit(..., method = "bootstrap")` and use `plotUncertainty()` for the corresponding parameter-uncertainty display.
